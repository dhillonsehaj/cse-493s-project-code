...
The replication for this project should be accessible in the `replication.ipynb` file and the extension should be accessible in the `extension.ipynb` file. The supplementary files should be located in the `dataset`, `dataset/split`, `dataset/extend`, and `src/baselines` folders. The notebooks should be able to be run to see all the results. Note: the original training code seems to have a typo using `return_attention_masks` which may have led to excessive outputs but was not modified to maintain the original code from the original authors.
